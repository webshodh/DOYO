...
const preprocessData = (data) => {
  ...
  dates.forEach(date => {
    const itemCounts = Array.from(menuItems).map(item => data[date][item] || 0);
...
const predictPopularity = async (inputs, outputs) => {
  ...
  return predictions.arraySync().map(row => row.map(value => Math.round(value)));
};
...
const MenuItemPopularityPrediction = () => {
  ...
  return (
    <div style={{ width: '80%', height: '400px', margin: '0 auto' }}>
      <h2>Menu Item Popularity Prediction</h2>
