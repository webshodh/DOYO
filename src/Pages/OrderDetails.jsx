...
const {
  ...
  const handleSearch = ({ name, mobileNo, date }) => {
    ...
    const filtered = orders.filter(order => 
      ...
      new Date(order.checkoutData.date).toISOString().split('T')[0] === formattedDate // Date comparison
    );

    if (filtered.length > 0) {
        const items = filtered.reduce((total, order) => 
          total + order.checkoutData.cartItems.reduce((sum, item) => sum + item.quantity, 0), 0);
    
        const price = filtered.reduce((total, order) => 
          total + order.checkoutData.cartItems.reduce((sum, item) => sum + (item.quantity * item.menuPrice), 0), 0);
      ...
      {filteredOrders.map(order => (
        <div key={order.orderId}>
          ...
          {order.checkoutData.cartItems.map(item => (
            <div key={item.uuid}>
              <p>{item.menuName} - {item.quantity} x {item.menuPrice}</p>
            </div>
          ))}
        </div>
      ))}
